...
`editable`이면 contentEditable 요소는 내용 높이만 차지하고, 그 아래 빈 영역(최소 40vh)이 붙습니다. 빈 영역을 누르면 글 끝에 커서를 둡니다. 마지막 블록이 이미지·구분선·표처럼 이어 쓸 수 없는 블록이면 그 아래에 새 문단을 만듭니다. contentEditable 요소를 크게 잡으면 태블릿 브라우저가 가상 키보드를 띄울 때 요소 전체를 보이게 하려고 페이지를 스크롤하기 때문입니다.
